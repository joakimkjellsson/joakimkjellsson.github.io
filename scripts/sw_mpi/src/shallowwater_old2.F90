...
PROGRAM SHALLOW
  ...
  INTEGER*4                              ::  id_nc, ierr, id_x, id_y,     &
      ...
      &                                      master = 0, column_type,     &
      &                                      rp, rm, ireq, column_type_v
  ...
  !!
  CALL MPI_TYPE_VECTOR(1, NY, 0, MPI_FLOAT, column_type, ierr_mpi)
  CALL err_mpi(ierr_mpi)
  
  CALL MPI_TYPE_COMMIT(column_type, ierr)
  CALL err_mpi(ierr_mpi)
  
  CALL MPI_TYPE_VECTOR(1, NY+1, 0, MPI_FLOAT, column_type_v, ierr_mpi)
  CALL err_mpi(ierr_mpi)
  
  CALL MPI_TYPE_COMMIT(column_type_v, ierr)
  CALL err_mpi(ierr_mpi)
  ...
  IF (rank /= master) THEN
     ...
     PRINT*,'f(1,5)',f(1,5),rank
     PRINT*,'f(NX+1,5)',f(NX+1,5),rank
     PRINT*,f(NX+1,1:NY)
     
     !! f(1,:) @ n to f(NX+1,:) @ n-1
     !CALL MPI_SENDRECV(f(1,:), 1, column_type, rm, rank, &
     !&                 f(NX+1,:), 1, column_type, rank, rank, &
     !&                 MPI_COMM_WORLD, ierr_mpi)
     !
     !CALL err_mpi(ierr_mpi)
     
     IF ( MOD(rank,2) == 0 ) THEN !Even rank
        PRINT*,' rank ',rank,' waiting to receive from ',rp,' with tag ',rp
        CALL MPI_RECV(f(NX+1,1:NY), 1, column_type, rp, rp, MPI_COMM_WORLD, ierr_mpi)
        CALL err_mpi(ierr_mpi)
        PRINT*,' rank ',rank,' received from ',rp,' with tag ',rp
        !PRINT*,' Receive posted on rank ',rank,' tag ',rp
        CALL MPI_SEND(f(1,1:NY), 1, column_type, rm, rank, MPI_COMM_WORLD, ierr_mpi) 
        CALL err_mpi(ierr_mpi)
        PRINT*,' rank ', rank,' sent to ',rm,' with tag ',rank
        !PRINT*,' Message sent to rank ',rm,' tag ',rank
        !CALL MPI_WAIT(ireq, istat, ierr_mpi)
        !PRINT*,' Waiting',rank
        !CALL err_mpi(ierr_mpi)
        PRINT*,' Done ',rank
     
     ELSE IF ( MOD(rank,2) /= 0 ) THEN !Odd rank
        PRINT*,' rank ',rank,' sending to ',rm,' with tag ',rank
        CALL MPI_SEND(f(1,1:NY), 1, column_type, rm, rank, MPI_COMM_WORLD, ierr_mpi)
        CALL err_mpi(ierr_mpi)
        PRINT*,' rank ',rank,' sent to ',rm,' with tag ',rank
        !PRINT*,' Receive posted on rank ',rank,' tag ',rp
        CALL MPI_RECV(f(NX+1,1:NY), 1, column_type, rp, rp, MPI_COMM_WORLD, ierr_mpi) 
        CALL err_mpi(ierr_mpi)
        PRINT*,' rank ', rank,' receiving from ',rp,' with tag ',rp
        !PRINT*,' Message sent to rank ',rm,' tag ',rank
        !CALL MPI_WAIT(ireq, istat, ierr_mpi)
        !PRINT*,' Waiting',rank
        !CALL err_mpi(ierr_mpi)
     ...
     END IF
     !CALL MPI_ISEND(f(1,:), 1, column_type, rm, rank, MPI_COMM_WORLD, ireq, ierr_mpi)
     !CALL err_mpi(ierr_mpi)
     ...
     PRINT*,'f(NX+1,5)',f(NX+1,5),rank
     PRINT*,'f(NX,5)',f(NX,5),rank
     
     !! f(NX,:) @ n to f(0,:) @ n+1
     !CALL MPI_SENDRECV(f(NX,:), 1, column_type, rp, rank, &
     !&                 f(0,:), 1, column_type, rank, rank, &
     !&                 MPI_COMM_WORLD, ierr_mpi)
     
     CALL MPI_IRECV(f(0,:), 1, column_type, rm, rm, MPI_COMM_WORLD, ireq, ierr_mpi)
     CALL err_mpi(ierr_mpi)
     ...
     PRINT*,'f(0,5)',f(0,5),rank
     
     !PRINT*,' rank ',rank,' sending to rank',rm
     !CALL MPI_SEND(f(1,:), 1, column_type, rm, rank, MPI_COMM_WORLD, ierr_mpi)
     !PRINT*,' rank ',rank,' will receive from ',rp
     !CALL MPI_RECV(f(NX+1,:), 1, column_type, rp, rp, MPI_COMM_WORLD, ierr_mpi)
     !PRINT*,' rank ',rank,' received from rank ',rp
     
     !PRINT*,' rank ',rank,' sending to ',rp
     !CALL MPI_SEND(f(NX,:), 1, column_type, rp, rank, MPI_COMM_WORLD, ierr_mpi)
     !PRINT*,' rank ',rank,' will receive from ',rm
     !CALL MPI_RECV(f(0,:), 1, column_type, rm, rm, MPI_COMM_WORLD, ierr_mpi)
     !PRINT*,' rank ',rank,' received from ',rm
  ...
  !!
  !! Wait for the netCDF file to be initialised
  !!
  CALL MPI_BARRIER(MPI_COMM_WORLD, ierr_mpi)
  CALL err_mpi(ierr_mpi)
